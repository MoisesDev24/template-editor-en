...
if (isset($_POST["imageURL"]) && !empty($_POST["imageURL"])) {
	...
	if (!$consulta_usuario) {
		mostrarError("Error querying the database.");
	}
		...
		if ($currentImageURL !== null) { // Verifica si hay una URL de imagen para este índice
			...
			if ($fila['numero_inserciones'] < $limitePeticiones) {
				...
				if (mysqli_num_rows($resultado_verificacion_existencia) > 0) {
					...
					mostrarError("A record with the same characteristics already exists.");
						...
						echo '
								<div class="alerta activo">
									<div class="alerta_contenido">
										...
										<div class="mensaje">
											<span class="text text-1" id="exito_alerta">Congrats</span>
				...
				mostrarError("The maximum allowed insertions per user has been reached.");
			}
		}
	...
	mostrarError("Insufficient data to process the request.");
} else {
	mostrarError("Insufficient data to process the request.");
}
...
function procesarImagen(&$imagen, $nombreCampo, $carpeta, $anchoMin, $altoMin, $anchoMax, $altoMax, $nombreUsuario)
{
	if (isset($_FILES[$nombreCampo])) {
		...
		$newCarpeta = 'https://platform.kalstein.net/template-editor/assets/img/users/' . $carpeta;
		$carpeta_f = '/home/he270716/public_html/platform.kalstein.net/template-editor/assets/img/users/' . $carpeta;

		$tiposPermitidos = array('image/jpg', 'image/jpeg', 'image/png', 'image/webp');

		// Validaciones
		if (!in_array(strtolower($tipo), $tiposPermitidos)) {
			mostrarError('File is not an image');
		} elseif ($tamano > 3 * 1024 * 1024) {
			mostrarError('The maximum allowed size is 3MB.');
		} elseif ($ancho > $anchoMax || $alto > $altoMax || $ancho < $anchoMin || $alto < $altoMin) {
			mostrarError("The image must have dimensions between {$anchoMin}x{$altoMin} and {$anchoMax}x{$altoMax}.");
		} else {
	...
	echo '
			<div class="alerta activo">
				<div class="alerta_contenido">
					...
					<div class="mensaje">
						<span class="text text-2">An error has ocurred!</span>
