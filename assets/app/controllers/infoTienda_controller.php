...
if (isset($_FILES['logo_tienda']) && $_FILES['logo_tienda']['error'] === UPLOAD_ERR_OK) {
	...
	if (!in_array(strtolower($tipo_f), $tipos_permitidos)) {
		mostrarError('The file is not an image');
		...
		mostrarError('The image must have dimensions between 600x250 and 220x50.');
	} else {
		...
		$logo_tienda = 'https://platform.kalstein.net/template-editor/assets/img/users/logos_tienda/' . $nombreUsuario[0] . '_' . $nombre_f;
	}
	...
	echo '
			<div class="alerta activo">
				<div class="alerta_contenido">
					...
					<div class="mensaje">
						<span class="text text-2">An error has occurred!</span>
						<span class="text">You havent added an image.</span>
					</div>
				</div>
		...
		echo '
				<div class="alerta activo">
					<div class="alerta_contenido">
						...
						<div class="mensaje">
				    		<span class="text text-2">An error has occurred!</span>
				    		<span class="text">Your registration was unsuccessful.</span>
						</div>
					</div>
